 ...
 */
package top.dex.fasterc.decice;
...
import top.dex.fasterc.fastercdex.FasterCdex;
import top.dex.fasterc.utils.FileUtils;

import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_EOF;
import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_PATCHES;
import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_PATH_CHECKSUM;
import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_PATH_EXISTS;
import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_PING;
import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_RESTART_ACTIVITY;
import static top.dex.fasterc.constants.ProtocolConstants.MESSAGE_SHOW_TOAST;
import static top.dex.fasterc.constants.ProtocolConstants.PROTOCOL_IDENTIFIER;
import static top.dex.fasterc.constants.ProtocolConstants.PROTOCOL_VERSION;
import static top.dex.fasterc.constants.ProtocolConstants.UPDATE_MODE_COLD_SWAP;
import static top.dex.fasterc.constants.ProtocolConstants.UPDATE_MODE_HOT_SWAP;
import static top.dex.fasterc.constants.ProtocolConstants.UPDATE_MODE_NONE;
import static top.dex.fasterc.constants.ProtocolConstants.UPDATE_MODE_WARM_SWAP;
...
public class Server {
    ...
    private class SocketServerReplyThread extends Thread {
        ...
        private void handle(DataInputStream input, DataOutputStream output) throws IOException {
            ...
            FasterCdex fasterCdex = FasterCdex.get(context);
            ...
            while (true) {
                ...
                switch (message) {
                    ...
                    case MESSAGE_PING: {
                        ...
                        long buildMillis = fasterCdex.getRuntimeMetaInfo().getBuildMillis();
                        output.writeLong(buildMillis);
                        String variantName = fasterCdex.getRuntimeMetaInfo().getVariantName();
                    ...
                    case MESSAGE_PATCHES: {
                        ...
                        try {
                            showToast("fasterc, receiving patch info...", context);
                        } catch (Throwable e) {
    ...
    private static boolean isResourcePath(String path) {
        return path.equals(ShareConstants.RESOURCE_APK_FILE_NAME) || path.equals(Paths.RESOURCE_FILE_NAME) || path.startsWith("res/");
    }
    ...
    private static boolean isDexPath(String path) {
        return path.endsWith(ShareConstants.DEX_SUFFIX);
    }
...
//        return updateMode;
        ...
        FasterCdex fasterCdex = FasterCdex.get(context);
        File workDir = new File(fasterCdex.getTempDirectory(), System.currentTimeMillis() + "-" + UUID.randomUUID().toString());
        try {
            for (ApplicationPatch change : changes) {
                String path = change.getPath();
            ...
            fasterCdex.getRuntimeMetaInfo().setPreparedPatchPath(workDir.getAbsolutePath());
            fasterCdex.getRuntimeMetaInfo().save(fasterCdex);
        } catch (Throwable e) {
    ...
    private static int handleResourcePatch(int updateMode, ApplicationPatch patch, String path, File workDir) throws IOException {
        ...
        FileUtils.writeFileFromString(new File(workDir, ShareConstants.RES_DIR + "/" + ShareConstants.RESOURCE_APK_FILE_NAME), Arrays.toString(patch.getBytes()),true);
    ...
    private int handleHotSwapPatch(int updateMode, ApplicationPatch patch, File workDir) {
        ...
        try {
            File tempDexDir = new File(workDir, ShareConstants.DEX_DIR + "/");
            FileUtils.writeFileFromString(new File(tempDexDir, patch.getPath()),Arrays.toString(patch.getBytes()),true);
        } catch (Throwable e) {
    ...
    private void restart(int updateMode, boolean hasDex, boolean incrementalResources, boolean toast) {
        ...
        if (!hasDex && incrementalResources && updateMode == UPDATE_MODE_WARM_SWAP) {
            ...
            File resDir = new File(FasterCdex.get(context).getRuntimeMetaInfo().getPreparedPatchPath(), ShareConstants.RES_DIR);
